...
public class PersonFXMLController implements Initializable {
    ...
    private void updateButtonAction(ActionEvent event) {
        ...
        //copia para el background thread
        ...
        Task<Person> updateTask = new Task<Person>(){
            ...
            protected Person call() throws Exception{
                Thread.sleep(1000);
                ftm.updatePerson(person); //actualizo la copia para el thread actual.
                return person;
            }
        };
        ...
        updateTask.setOnSucceeded(t->{
            backgroundActive.set(backgroundActive.get() - 1);
            logger.log(Level.WARNING,"Update Task finished : {0}", t.getSource().getValue());
            logger.log(Level.FINE, "Number of background threads: {0}", backgroundActive.get() );
        });
        
        updateTask.setOnFailed(t->{
            backgroundActive.set(backgroundActive.get() - 1);
            logger.log(Level.FINE,"Update Task failed : {0}", t.getSource().getException() != null ? t.getSource().getException() : "unknown failure");
            logger.log(Level.FINE, "Number of background threads: {0}", backgroundActive.get() );            
        });
    ...
    private void processAllButtonAction(ActionEvent event) {
        ...
        final ProcessAllTask processTask = new ProcessAllTask(FXCollections.observableArrayList(ftm.getAllPeople()));
        ...
        processTask.setOnSucceeded(t->{
            logger.log(Level.FINE,"Process all task finish {0}", t.getSource().getValue());
            resetUI();
        });
        
        processTask.setOnFailed(t->{
            logger.log(Level.WARNING,"Process all task failed {0}", t.getSource().getException() != null ? t.getSource().getException() : "Unknown Failure");
            resetUI();
        });
        
        processTask.setOnCancelled(t->{
            logger.log(Level.FINE,"Process all task canceled");
            resetUI();
        });
    ...
    //Private inner class para ejecutar tasks
    private class ProcessAllTask extends Task<String> {
        ...
        protected String call() throws Exception {
            ...
            logger.log(Level.FINE, "Process List : {0}",processList);
            int taskProgress = 0;
